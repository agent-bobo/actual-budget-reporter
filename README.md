# Budget Reporter - Actual Budget 智能周报系统

> 🚧 **Work in Progress** - 当前为 MVP 阶段，API 连接部分需要验证

智能财务周报生成器，与 Actual Budget 集成，每周自动分析消费数据并推送到 Discord。

**Repo:** https://github.com/agent-bobo/actual-budget-reporter

## ✨ 特性

- 📊 **零 Token 浪费** - 纯规则引擎计算指标，仅 Insight 使用 Gemini
- 🔔 **自动推送** - 每周日自动发送报告到 Discord
- 🔍 **异常检测** - 自动标记支出激增、大额消费、未分类交易
- 💰 **预算追踪** - 对比月度预算，预警超支风险
- 🧠 **智能摘要** - Gemini 生成自然语言财务洞察

## 🏗️ 架构

```
┌─────────────────┐     ┌──────────────────┐     ┌───────────────┐
│  Actual Budget  │────▶│  Budget Reporter │────▶│   Discord     │
│   (你的账本)     │     │  ├─ Actual API   │     │  (手机通知)   │
│                 │     │  ├─ 规则引擎      │     │               │
│                 │     │  ├─ Gemini摘要   │     │               │
└─────────────────┘     └──────────────────┘     └───────────────┘
```

**成本对比:**

| 方案 | 每周 Token 消耗 | 月成本 (Gemini Flash) |
|------|-----------------|----------------------|
| 每笔交易走 LLM (actual-ai 模式) | ~50,000 | ~$0.10 |
| **本方案 (预聚合+摘要)** | **~500** | **~$0.001** |

## 🚀 快速开始

### 1. 获取 Actual Budget 配置

登录 Actual Budget → Settings → Show advanced settings → **Sync ID** 复制下来

### 2. 创建 Discord Webhook

Discord 服务器 → 设置 → Integrations → Webhooks → **New Webhook**

复制 Webhook URL

### 3. 获取 Gemini API Key (可选)

访问 [Google AI Studio](https://aistudio.google.com/app/apikey) 创建 API Key

> 不设置也能运行，只是没有 AI 生成的自然语言摘要

### 4. 配置环境变量

```bash
cp .env.example .env
# 编辑 .env 填入真实值
```

### 5. 启动

```bash
# 和 Actual Budget 一起启动
docker-compose up -d actual-server

# 测试运行一次
docker-compose run --rm budget-reporter
```

### 6. 设置定时任务 (Cron)

```bash
# 每周日早上 10 点自动运行
0 10 * * 0 cd /path/to/budget-reporter && ./run-weekly.sh
```

或使用 Docker 健康检查方案：

```yaml
# 在 docker-compose 中添加 scheduler 服务
  scheduler:
    image: mcuadros/ofelia:latest
    depends_on:
      - budget-reporter
    command: daemon --docker
    volumes:
      - /var/run/docker.sock:/var/run/docker.sock:ro
    labels:
      ofelia.job-run.budget-report.schedule: "0 10 * * 0"
      ofelia.job-run.budget-report.container: "budget-reporter"
```

## 📋 配置说明

### 环境变量

| 变量 | 必需 | 说明 |
|------|------|------|
| `ACTUAL_PASSWORD` | ✅ | Actual Budget 登录密码 |
| `ACTUAL_BUDGET_ID` | ✅ | Budget Sync ID (设置里找) |
| `DISCORD_WEBHOOK_URL` | ✅ | Discord Webhook URL |
| `GOOGLE_GENERATIVE_AI_API_KEY` | ❌ | Gemini API Key |
| `MONTHLY_BUDGET` | ❌ | 月度预算，JSON格式(单位:分) |

### 月度预算配置示例

```bash
MONTHLY_BUDGET={"餐饮":50000,"交通":20000,"购物":30000}
# 表示: 餐饮$500/月, 交通$200/月, 购物$300/月
```

## 📊 报告示例

```markdown
# 📊 本周财务简报
**2024-01-14 ~ 2024-01-20**

## 💰 收支概览
• 收入: **$3,500**
• 支出: **$1,200** (日均 $171)
• 结余: **$2,300**

## 📈 支出Top3
1. 餐饮: $450
2. 交通: $200
3. 购物: $150

## ✅ 预算状态
预算进度正常

## 💡 本周洞察
本周餐饮支出占收入的13%，处于健康范围。建议继续保持每周
预算节奏，月底预计结余$2000+，可以考虑增加储蓄目标。

## 🚨 需要关注
• 周三有一笔$299的电子产品支出
```

## 🛠️ 开发

```bash
# 本地开发
# Local Development
uv sync

# Run locally
uv run python -m src.reporter
```

## 📁 项目结构

```
budget-reporter/
├── src/
│   ├── __init__.py
│   ├── actual_client.py    # Actual Budget API 客户端
│   ├── analyzer.py         # 财务分析引擎 (纯规则)
│   ├── gemini_summarizer.py # Gemini 摘要生成
│   ├── discord_notifier.py # Discord 推送
│   └── reporter.py         # 主程序
├── Dockerfile
├── docker-compose.yml
├── pyproject.toml
├── uv.lock
├── .env.example
├── run-weekly.sh           # Cron 脚本
└── README.md
```

## 🔒 隐私说明

- 原始交易数据**不会**发送到 LLM
- 仅发送预聚合的统计指标 (总额、分类占比等)
- Gemini 仅用于生成 3-5 句话的摘要
- 所有数据在本地处理，不存储到任何第三方

## 📝 路线图

- [x] 基础周报生成
- [x] 异常检测 (环比/大额/未分类)
- [x] Discord 推送
- [x] Gemini 智能摘要
- [ ] 预算健康度追踪
- [ ] 月度趋势报告
- [ ] 异常实时提醒 (另一个 cron job)
- [ ] 自然语言查询接口 (未来的方向)

## License

MIT
